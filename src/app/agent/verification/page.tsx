      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-6 pb-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-100 pb-5">
          <div className="space-y-1">
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight leading-none uppercase italic">Agent <span className="text-green-700">Verification</span></h1>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            <Button size="sm" className="h-9 px-5 rounded-xl font-black shadow-lg shadow-green-700/20 bg-green-700 hover:bg-green-800 gap-2 text-[10px] w-full sm:w-auto text-white border-none">
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4">
            <Card className="rounded-[1.5rem] bg-slate-900 text-white p-6 relative overflow-hidden shadow-xl border-none h-full flex flex-col justify-center">
              <div className="absolute top-0 right-0 w-24 h-24 bg-green-500/20 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl" />
              ...
              <div className="space-y-4 bg-white/5 p-5 rounded-xl border border-white/10 relative z-10">
                <div className="flex justify-between items-end mb-1">
                  ...
                  <span className="text-2xl font-black text-green-400 tracking-tighter italic">{auditScore}%</span>
                </div>
            ...
            <Card className="shadow-sm border-slate-100 rounded-[2rem] bg-white overflow-hidden">
              <CardHeader className="p-5 border-b border-slate-50 flex flex-row items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-9 w-9 rounded-xl bg-green-50 flex items-center justify-center text-green-700 shadow-inner">
                ...
                <div className="divide-y divide-slate-50">
                  {agentDocs.map((doc) => (
                    <div key={doc.id} className="group flex items-center gap-4 px-6 py-4 hover:bg-slate-50/50 transition-all cursor-pointer">
                      <div className="h-10 w-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-300 group-hover:bg-green-700 group-hover:text-white transition-all shadow-inner shrink-0">
                        <FileText className="h-5 w-5" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-[13px] font-black text-slate-800 leading-none group-hover:text-green-700 transition-colors">{doc.name}</p>
                          <Badge variant="outline" className="text-[7px] h-3.5 px-1.5 border-green-50 text-green-400 font-bold uppercase tracking-widest">{doc.type}</Badge>
                        </div>
                      ...
                      <div className="flex items-center gap-4 shrink-0">
                        ...
                        <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl bg-slate-50 text-slate-400 hover:text-green-700 hover:bg-white shadow-inner transition-all border border-transparent hover:border-green-100">
...
function StatusCard({ icon, label, value, sub }: any) {
  return (
    <Card className="p-4 rounded-2xl border-slate-100 shadow-sm bg-white group hover:border-green-200 transition-all">
      <div className="flex items-center gap-3">
        <div className="h-9 w-9 rounded-xl bg-slate-50 text-slate-400 group-hover:bg-green-700 group-hover:text-white transition-all flex items-center justify-center shadow-inner shrink-0">
          {icon}
        </div>
        <div className="min-w-0">
          ...
          <p className="text-[7px] font-bold text-green-700 uppercase tracking-widest mt-1">{sub}</p>
        </div>
      </div>
    </Card>
  );
